...
![Agent in Foundry](../../media/Solution/0202_AgentInFoundry1.png)
...
To do so, open `/src/chat_app.py`. First, comment out **lines 45 and 250-256**, which deal with the single-agent example. A keyboard shortcut to comment out multiple lines in Visual Studio Code is to select the lines you want to comment (or uncomment) and then press `CTRL + /` on Windows or `CMD + /` on Mac.
